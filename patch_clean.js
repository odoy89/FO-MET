...
// 1. State
code = code.replace('const [savingForm, setSavingForm] = useState(false);',
...
// 3. handleEditRow
code = code.replace(/fotoUrl:\s*row\[14\]\s*\|\|\s*"",\r?\n\s*foto:\s*null,\r?\n\s*peruntukan:\s*row\[11\]/,
  'fotoUrl: row[14] || "",\n      fotoSegelKiriUrl: row[15] || "",\n      fotoSegelKananUrl: row[16] || "",\n      foto: null,\n      fotoSegelKiri: null,\n      fotoSegelKanan: null,\n      peruntukan: row[11]'
);

code = code.replace(/rowNumber:\s*row\[15\]\s*\|\|\s*null/, 'rowNumber: row[17] || null');
...
`;
code = code.replace(/payloadArray\.push\(\{\r?\n\s*tanggal/, uploadKiriKanan + '\n        payloadArray.push({\n          tanggal');

code = code.replace(/fileUrl,\r?\n\s*fotoUrl,\r?\n\s*peruntukan/, 'fileUrl,\n          fotoUrl,\n          fotoSegelKiriUrl,\n          fotoSegelKananUrl,\n          peruntukan');

// 5. handleMarkSudah
code = code.replace('const rowNumber = r[15];', 'const rowNumber = r[17];');

// 6. CSV Export headers
code = code.replace(/"File PK",\s*"Status",/, '"File PK",\n      "Foto KWh",\n      "Foto Segel Kiri",\n      "Foto Segel Kanan",\n      "Status",');
code = code.replace(/r\[10\] \|\| "",\r?\n\s*r\[12\] \|\| "",/, 'r[10] || "",\n        r[14] || "",\n        r[15] || "",\n        r[16] || "",\n        r[12] || "",');

// 7. Data table headers and rendering
code = code.replace(/<th>Foto KWh<\/th>\r?\n\s*<th>Status<\/th>/, '<th>Foto KWh</th>\n                  <th>Foto Segel Kiri</th>\n                  <th>Foto Segel Kanan</th>\n                  <th>Status</th>');
...
code = code.replace(/<td>\s*\{typeof r\[14\] === "string"[\s\S]*?<\/td>/, tbodyImageRendering);


// 8. Add Data Button and Modal wrapping
code = code.replace(/<div className="table-area" id="table-area">/, 
...
code = code.replace(/setFormRows\(\[obj\]\);\r?\n\s*if \(typeof window !== "undefined"\) \{\r?\n\s*window\.scrollTo\(\{ top: 0, behavior: "smooth" \}\);\r?\n\s*\}/, 'setFormRows([obj]);\n    setShowModalForm(true);');


// Modal UI Form Area (careful with regex)
const formAreaRegex = /<div className="form-area" id="form-area">[\s\S]*?<\/form>\r?\n\s*<\/div>/;
...
if (formAreaMatch) {
  ...
  formContent = formContent.replace(/<\/form>\r?\n\s*<\/div>/, '</form>\n        </div>\n      </div>\n    </div>\n  )}');
  ...
  formContent = formContent.replace(/<div className="col-md-2">\r?\n\s*<label>Upload Foto KWh<\/label>[\s\S]*?<\/div>/, photoInputHTML);
...
code = code.replace(/Swal\.fire\(\{\r?\n\s*icon: "success",\r?\n\s*title: "Berhasil!",\r?\n\s*text: "Data berhasil disimpan.",/, 
  'setShowModalForm(false);\n      Swal.fire({\n        icon: "success",\n        title: "Berhasil!",\n        text: "Data berhasil disimpan.",'
);

// 9. CSS
code = code.replace(/\.form-area,\r?\n\.table-area \{/, 
  `.modal-overlay {
  position: fixed;
...
code = code.replace(/\.row\.align-items-end\.g-2\.mb-2 > div \{[\s\S]*?\}/, 
  `/* Update modal di HP */
  .modal-overlay {
    padding: 10px;
  }
...
console.log("Success patching clean dashboard.js");
